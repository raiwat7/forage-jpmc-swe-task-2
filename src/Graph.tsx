...
class Graph extends Component<IProps, {}> {
  ...
  componentDidMount() {
    ...
    if (this.table) {
      ...
      // Adding more attributes of the element
      elem.load(this.table);
      // since we want a continuous line graph we’re using a y_line.
      elem.setAttribute('view', 'y_line');
      // ‘column-pivots’ is what will allow us to distinguish stock ABC from DEF. We
      // use ‘[“stock”]’ as its value here.
      ...
      // i.e. :top_ask_price, top_bid_price, stock, timestamp. For this instance
      // we only care about top_ask_price
      elem.setAttribute('columns', '["top_ask_price"]');
      // ‘aggregates’ allows us to handle the duplicated data we observed earlier and
      // consolidate it into a single data point. In our case we only want to consider
      // a data point unique if it has a unique stock name and timestamp. If there are
